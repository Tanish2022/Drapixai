...
const shellLineEndingVerifier = read('scripts/verify-shell-line-endings.mjs');
...
const stagingSecretGenerator = read('deploy/scripts/generate-staging-secrets.py');
const stagingDatabaseRole = read('deploy/staging/provision-database-role.sh');
assertIncludes(stagingSecretGenerator, 'postgresql://drapixai_staging_api:', 'Staging API must use a separate database role');
assertIncludes(stagingDatabaseRole, 'NOSUPERUSER NOCREATEDB NOCREATEROLE', 'API database role must not retain administrative privileges');
assertIncludes(stagingDatabaseRole, 'GRANT SELECT, INSERT ON TABLE public."SecurityAuditLog"', 'Runtime audit access must be append/read only');
...
const containerVex = read('deploy/security/vex/drapixai-container.openvex.json');
...
const workstationSecurityCandidate = read('deploy/workstation/build-security-candidate.sh');
const aiRequirements = read('drapixai_ai/requirements.txt');
const aiRtxRequirements = read('drapixai_ai/requirements.rtx-pro-6000.txt');
const aiRtxSecurityCandidate = read('drapixai_ai/requirements.rtx-pro-6000.security-candidate.txt');
...
const auditTruncateMigration = read('apps/api/prisma/migrations/20260907010000_block_security_audit_truncate/migration.sql');
const auditImmutabilityVerifier = read('apps/api/src/scripts/verify-audit-immutability.ts');
...
assertIncludes(launchGates, 'shell-line-endings', 'Launch gates must reject shell scripts that are unsafe to execute on Linux.');
assertIncludes(shellLineEndingVerifier, "['ls-files', '-z', '--', '*.sh']", 'Shell line-ending validation must inspect the tracked shell-script set.');
assertIncludes(shellLineEndingVerifier, 'contents.includes(0x0d)', 'Shell line-ending validation must reject carriage-return bytes.');
...
assertIncludes(aiDockerfile, "rglob('*.sh')", 'AI image builds must inspect every copied deployment shell script.');
assertIncludes(aiDockerfile, "if b'\\r' in path.read_bytes()", 'AI image builds must reject carriage-return bytes in shell scripts.');
assertIncludes(aiDockerfile, "-exec bash -n {} \\;", 'AI image builds must syntax-check copied deployment shell scripts.');
...
assertIncludes(aiRtxRequirements, 'fastapi==0.139.0', 'RTX production ingress must resolve a patched Starlette runtime');
assertIncludes(aiRtxRequirements, 'uvicorn[standard]==0.40.0', 'RTX production must pin the audited ASGI server release');
assertIncludes(aiRtxRequirements, 'urllib3==2.7.0', 'RTX production outbound HTTP must use the patched urllib3 release');
assertIncludes(aiRtxRequirements, 'msgpack==1.2.1', 'RTX production must use the patched MessagePack parser');
...
assertIncludes(auditTruncateMigration, 'BEFORE TRUNCATE ON "SecurityAuditLog"', 'Audit protection must cover table truncation');
assertIncludes(auditTruncateMigration, 'FOR EACH STATEMENT', 'Truncation protection must use a statement-level trigger');
assertIncludes(auditImmutabilityVerifier, "expectMutationRejected('TRUNCATE'", 'Database certification must test actual truncation rejection');
assertIncludes(auditImmutabilityVerifier, 'I_ACKNOWLEDGE_DISPOSABLE_DATABASE', 'Destructive audit tests must require disposable-database approval');
...
assertIncludes(aiDockerfile, 'FROM ${DRAPIXAI_AI_BUILD_IMAGE} AS builder', 'AI dependencies must compile outside the production runtime stage');
assertIncludes(aiDockerfile, 'FROM ${DRAPIXAI_AI_BASE_IMAGE}', 'AI production must use the hardened runtime stage');
...
assertIncludes(aiDockerfile, 'apt-get purge -y linux-libc-dev', 'AI serving images must not retain kernel development headers.');
assertIncludes(aiDockerfile, '/usr/local/lib/python3.12/dist-packages/pillow-*', 'AI serving images must remove the stale base Pillow distribution.');
assertIncludes(aiDockerfile, '/usr/local/lib/python3.12/dist-packages/urllib3-*', 'AI serving images must remove the stale base urllib3 distribution.');
assertIncludes(aiDockerfile, "module_path.is_relative_to(venv)", 'AI image builds must prove security-sensitive Python imports resolve from the patched venv.');
assertIncludes(aiDockerfile, "'pillow': '12.3.0'", 'AI image builds must assert the patched Pillow version.');
assertIncludes(aiDockerfile, "'urllib3': '2.7.0'", 'AI image builds must assert the patched urllib3 version.');
...
assertIncludes(containerScanScript, 'DRAPIXAI_TRIVY_TIMEOUT:-45m', 'Container scanning must allow enough time for the production ML runtime.');
assertIncludes(containerScanScript, '^[1-9][0-9]*(s|m|h)$', 'Container scanning must reject unbounded or malformed timeout overrides.');
assertIncludes(containerScanScript, '--timeout "${TRIVY_TIMEOUT}"', 'Every Trivy invocation must enforce the bounded ML-image scan timeout.');
assertIncludes(containerScanScript, '"scanTimeout"', 'Container-scan evidence must record the effective scanner timeout.');
assertIncludes(containerScanScript, '--vex /scan/vex.openvex.json', 'Container scanning must apply the reviewed version-specific VEX document.');
assertIncludes(containerScanScript, '"vexSha256"', 'Container-scan evidence must bind the reviewed VEX document by digest.');
assertIncludes(containerScanScript, 'type=bind,src=${scratch},dst=/tmp', 'Large ML image scans must use private per-run disk scratch space.');
assertNotIncludes(containerScanScript, '--tmpfs /tmp:', 'Container scanning must not impose a fixed RAM-only file-size ceiling on PyTorch libraries.');
assertIncludes(containerVex, 'CVE-2026-14456', 'The reviewed OpenSSL VEX declaration must identify the scanner finding exactly.');
assertIncludes(containerVex, 'libssl3@3.0.20-1~deb12u2', 'The OpenSSL VEX declaration must remain pinned to the reviewed package version.');
assertIncludes(containerVex, 'pkg:pypi/msgpack@1.1.2', 'Deleted-layer MessagePack VEX must remain pinned to the exact absent distribution.');
assertIncludes(containerVex, 'GHSA-6v7p-g79w-8964', 'Deleted-layer MessagePack VEX must identify the exact scanner finding.');
assertIncludes(containerVex, 'pkg:pypi/setuptools@70.3.0', 'Deleted-layer Setuptools VEX must remain pinned to the exact absent distribution.');
assertIncludes(containerVex, 'CVE-2025-47273', 'Deleted-layer Setuptools VEX must identify the exact scanner finding.');
assertIncludes(containerVex, 'vulnerable_code_not_present', 'Every container VEX declaration must record why the exact package is not affected.');
