...
assertIncludes(dashboardPage, 'apiKey: previewPayload.token', 'Dashboard SDK preview must use only the short-lived preview credential');
...
assertIncludes(sdkReactDoc, 'NEXT_PUBLIC_API_BASE_URL', 'React SDK docs must direct production storefronts to the configured API base URL');
assertIncludes(sdkReactDoc, 'https://api.drapixai.com', 'React SDK docs must show an HTTPS production API endpoint example');
assertIncludes(sdkReactDoc, 'confirmed `productId`', 'React SDK docs must require confirmed product mappings');
assertIncludes(sdkReactDoc, 'cached try-on asset', 'React SDK docs must describe cached garment try-on usage');
assertIncludes(sdkReactDoc, 'Do not ship a public storefront that points to localhost', 'React SDK docs must warn against shipping localhost/tunnel API URLs');
assertNotIncludes(sdkReactDoc, 'default `http://localhost:8000`', 'React SDK docs must not present localhost as the production default base URL');
...
assertIncludes(apiDockerfile, 'gcr.io/distroless/nodejs20-debian12:nonroot', 'API runtime must exclude unnecessary shell and package-manager tooling');
assertIncludes(webDockerfile, 'gcr.io/distroless/nodejs20-debian12:nonroot', 'Web runtime must exclude unnecessary shell and package-manager tooling');
