...
PyTorch 2.12.1 requires `setuptools<82`, while `PYSEC-2026-3447` is fixed only in Setuptools 83. The advisory concerns Unicode-normalization bypasses while packaging source distributions on macOS. DrapixAI production runs on Ubuntu Linux, installs a prebuilt PyTorch wheel, does not build or publish source distributions at runtime, and never accepts package-manifest input from users. CI therefore suppresses that ID as deployment-inapplicable. These three named IDs are the complete exception list; all other direct and transitive packages must pass the resolved dependency audit.
